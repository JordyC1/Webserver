...
// Manejar peticin OPTIONS (preflight)
if ($_SERVER['REQUEST_METHOD'] === 'OPTIONS') {
    http_response_code(200);
    exit();
}

// Configuraci de la base de datos
...
// Obtener datos JSON del cuerpo de la peticin
...
if ($alerta_id <= 0) {
    die(json_encode(["success" => false, "message" => "ID de alerta invlido"]));
}
...
try {
    ...
    if ($estado === 'activa') {
        // Si se reactiva, limpiar fecha y notas de resolucin
        $sql = "UPDATE alertas 
                SET estado = ?, 
